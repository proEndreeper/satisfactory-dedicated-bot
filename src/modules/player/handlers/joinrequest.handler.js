const debug = require("debug")("module-players");
const {SatisfactoryUser} = require("../satisfactoryuser.js");

let MessageReference = 0;

function log(txt)
{
	debug(`[JoinRequest-${MessageReference}] ${txt}`)
}

// Example console output
// [2021.10.30-15.50.46:530][872]LogNet: Join request: /Game/FactoryGame/Map/DedicatedserverEntry?EntryTicket=?Name=ProEndreeper?SplitscreenCount=1
const JoinRequestRegex = /^\[([^\]]+)\]\[([^\]]+)\]LogNet: Join request: \/Game\/FactoryGame\/Map\/DedicatedserverEntry\?EntryTicket=([0-9A-F]+)\?Name=(.+)\?.+/;

module.exports = async (msg, socket, moduleData) => {
	if(!JoinRequestRegex.test(msg)) return;

	log("Received join request!");
	let data = JoinRequestRegex.exec(msg);
	let entryTicket = data[3];
	let userName = data[4];
	let player = moduleData.playerSessions.filter((player)=>player.entryTicket==entryTicket)[0];

	if(!(player instanceof SatisfactoryUser))
	{
		log("Failed to find player associated with entry ticket.");
		MessageReference++;
		return;
	}

	log(`User '${player.name}' is joining!`);

	player.lastStateChange = data[1];
	player.joinRequestId = data[2];

	MessageReference++;

	socket.emit("player join pending",{
		player
	});
}